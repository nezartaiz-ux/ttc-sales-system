...
CREATE POLICY "Sales staff, accountants and admins can manage invoices" ON public.sales_invoices FOR ALL USING (
  public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'sales_staff') OR public.has_role(auth.uid(), 'accountant')
);
...
BEGIN
  ...
  new_role := CASE 
    WHEN NEW.email IN ('nezar@tehama.com', 'nezraraiz@gmail.com', 'nezartaiz@gmail.com') THEN 'admin'::user_role
